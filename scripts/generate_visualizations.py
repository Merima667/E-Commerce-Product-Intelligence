...
Usage (from the project root):
    ...
    python scripts/generate_visualizations.py --data path/to/products_raw.csv

The script:
  1. Loads the cleaned e-commerce products dataset (data/processed/analytics/products_raw.csv)
...
def main():
    parser = argparse.ArgumentParser(
        description="Lab 12 – E-Commerce Product Intelligence Data Visualization Generator"
    )
    parser.add_argument(
        "--data",
        default=str(ROOT / "data" / "processed" / "analytics" / "products_raw.csv"),
        help="Path to the cleaned products CSV (default: data/processed/analytics/products_raw.csv)",
    )
